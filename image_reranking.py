...
def MatchFeatures(query_locations,
  ...
  print(index_image_descriptors.shape, query_descriptors.shape)
...
def RerankByGeometricVerification(idx, input_ranks, initial_scores, query_name,
                                  index_names, feature_location,
                                  feature_descriptor, junk_ids):
  ...
  """
  input_ranks = input_ranks[:, idx]
  initial_scores = initial_scores[:, idx]
  query_name = query_name[idx]
  ...
  #     query_features_path)
  query_locations = feature_location[query_name]
  query_descriptors = feature_descriptor[query_name]
  ...
  for i in range(num_to_rerank):
    ...
    image_name = index_names[index_image_id]

    index_image_locations = feature_location[image_name]
    index_image_descriptors = feature_descriptor[image_name]
